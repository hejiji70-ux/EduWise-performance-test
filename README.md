...
| 场景 | 线程数 | 循环次数 | 覆盖接口 |
|---|---|---|---|
| 场景 1：三端登录压测 | 5 | 2 | Admin/Teacher/App 登录 |
| 场景 2：Admin 后台负载 | 3 | 3 | 用户信息、学员列表、讲师列表、课程列表、统计 |
| 场景 3：App 端业务负载 | 5 | 3 | Banner、分类、课程搜索、个人信息、订单 |
| 场景 4：Teacher 端 | 3 | 3 | 登录、个人信息、学科、课程列表 |
...
推送到 GitHub 后，Actions 自动验证 JMX 脚本格式正确性。

## 注意事项

1. Docker 必须已启动：确保 `http://localhost:9096` 可访问
2. App 端限流严格：原系统有 `@AccessLimit(maxCount=1, seconds=120)` 限流，高并发可能触发限流返回错误（属正常行为）
